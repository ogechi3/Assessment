      ...
      {list.map((item) => (
        <div key={item.id} className="flex flex-row gap-3">
          ...
          <div className="">
            ...
            <div className="font-bold  text-[10px] md:text-base">{item.value}</div>
            <div className="font-light text-[10px] rounded-md hidden md:flex" style={{backgroundColor:item.button}}>{item.status}</div>
          </div>
        </div>
      ))}
    </div>
